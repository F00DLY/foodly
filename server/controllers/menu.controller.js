...
const updateManu = asynchandler(async (req, res) => {
  ...
  const removeMenuItem = asynchandler(async (req, res) => {
    try {
      ...
      const itemIndex = restaurantMenu.menuItems.findIndex(item => menuItemId.toString() === menuItemId);
      ...
      console.error('Error in Remove Menu Item Controller:', error);
      if (error instanceof ApiError) {
  ...
  const showMenu = asynchandler(async (req, res) => {
    try {
      ...
      if (!Restaurantname) {
        throw new ApiError(400, 'Restaurant name is required in the request body.');
      }
