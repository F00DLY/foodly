...
const registerRestaurant = asynchandler(async (req, res) => {
  ...
  if (![Restaurantname, email, password, mobail].every((field) => field.trim() !== "")) {
  ...
  if (!createdRestaurant) {
    throw new ApiError(500, "Something went wrong when registering the restaurant");
  }

  return res.status(201).json(new Apiresponse(200, createdRestaurant, "Restaurant registered successfully"));
});
...
const loginRestaurant = asynchandler(async (req, res) => {
  ...
  const { accessToken, refreshToken } = await genrateAccessAndRefrereshToken(restaurant._id);
  ...
  return res.status(200)
    ...
    .json(new Apiresponse(200, { restaurant: loggedInRestaurant, accessToken, refreshToken }, "Restaurant logged in successfully"));
});
...
try {
   const decodedToken=jwt.verify(incomingrefreshToken,
     ...
     const{accessToken,newrefreshToken}= await genrateAccessAndRefrereshToken(restaurant._id);
   ...
   .json(
       new Apiresponse(
           200,{accessToken,newrefreshToken},"Accesstoken refreshed"
       )
   )
} catch (error) {
...
const deletresturentaccount=asynchandler(async(req,res)=>{
  try {
    ...
    const deletedresturent = await Restaurant.findByIdAndDelete(resturentId);
    if (!deletedresturent) {
        throw new ApiError(404, 'account   not found');
      }
      res.status(200).json({ message: 'account deleted   successfully' });
  } catch (error) {
    throw new ApiError(400,"account not deleted")
  }
  });
